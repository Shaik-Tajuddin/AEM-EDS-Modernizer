...
**Repeatable patterns (screenshots / requirements):** Model as a **parent** block plus a **child** block (one child instance per repeat). **Non-repeated** fields and **overall** appearance belong on the **parent**; **per-item** fields and **per-item** appearance belong on the **child**. The **parent’s `filters` → `components` must list the child block** so the child can be placed inside the parent. **Nested** child blocks may get **different row wrapper HTML** than the same block at section level — child **`extractConfig`** may use `getHtmlFromRow` / `getTextFromBlockRow` when needed; the parent stays a normal block unless it integrates children. See `.cursor/rules/22-repeatable-parent-child-blocks.mdc`.
