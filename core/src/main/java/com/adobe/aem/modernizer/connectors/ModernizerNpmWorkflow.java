...
public final class ModernizerNpmWorkflow {
    ...
    public static final String YAML =
            ...
            + "    inputs:\n"
            + "      command:\n"
            + "        description: 'npm script (lint, lint:fix, build:json, or lint:fix,build:json). On push this defaults to lint.'\n"
            + "        required: false\n"
            + "        default: 'lint'\n"
            + "permissions:\n"
            + "  contents: write\n"
            ...
            + "          if [ \"$CMD\" = \"lint:fix,build:json\" ] || [ \"$CMD\" = \"build\" ] || [ \"$CMD\" = \"lint:fix & build:json\" ]; then\n"
            + "            npm run lint:fix || true\n"
            + "            npm run build:json || true\n"
            + "          else\n"
            + "            npm run \"$CMD\"\n"
            + "          fi\n"
            + "      - name: Commit build & lint output\n"
            + "        if: github.event_name == 'workflow_dispatch' && (github.event.inputs.command == 'lint:fix' || github.event.inputs.command == 'build:json' || github.event.inputs.command == 'lint:fix,build:json' || github.event.inputs.command == 'build' || github.event.inputs.command == 'lint:fix & build:json')\n"
            + "        run: |\n"
